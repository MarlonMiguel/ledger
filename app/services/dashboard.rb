class Dashboard
  ...
  def top_person
    Rails.cache.fetch("top_person", expires_in: 1.hours) do
    Person.order(:balance).last
    end
  end
  
  def bottom_person
    Rails.cache.fetch("bottom_person", expires_in: 1.hours) do
  ...
  def active_people_ids
    Rails.cache.fetch("active_people_ids", expires_in: 1.hours) do
  ...
  def balance
    Rails.cache.fetch("balance", expires_in: 1.hours) do  
